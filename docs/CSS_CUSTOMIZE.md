...
`data/theme/compose.css` でカスタマイズできます（字幕ウィンドウ・画像ポップアップと同じ仕組みです）。基本配色はメイン画面の現在のライト/ダーク設定に自動で追従し、そのうえに `compose.css` の内容が重ねて適用されます。

主なクラス/ID（プレーンな HTML のため、クラスではなく ID で指定する箇所があります）:

| セレクタ | 要素 |
|---|---|
| `body.dark` | ダークモード時に付与される (メイン画面の `.dark` と同様) |
| `#header` / `#mode-row` / `.mode-btn` | ヘッダー / モード切替行 / 「レス」「新スレ」ボタン |
| `#target` | 対象スレ名の表示 |
| `#subject` | スレッドタイトル入力欄 (新スレ時のみ表示) |
| `#name` / `#mail` / `#sage` | 名前 / メール / sage チェックボックス |
| `#body` | 本文入力欄 |
| `#meta` (`#charcount` / `#linecount`) | 文字数・行数表示 |
| `#result` (`.ok` / `.err`) | 投稿結果表示 |
| `#actions` / `#submit` / `#close` | フッター行 / 送信ボタン / 閉じるボタン |

ウィンドウのサイズは、変更するとその大きさを記憶し、次回開いたときに復元します（アプリを再起動すると初期化されます）。
